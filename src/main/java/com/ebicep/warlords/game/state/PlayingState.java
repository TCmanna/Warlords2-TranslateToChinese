...
public class PlayingState implements State, TimerDebugAble {
    ...
    public void end() {
        ...
        //PUBS
        if (!game.getAddons().contains(GameAddon.PRIVATE_GAME) && !game.getAddons().contains(GameAddon.IMPOSTER_MODE) && winEvent != null && game.playersCount() >= 2) {
        ...
        else if (RecordGamesCommand.recordGames && !game.getAddons().contains(GameAddon.IMPOSTER_MODE) && winEvent != null && game.playersCount() >= 2 && timer >= 60) {

            DatabaseGameBase.addGame(game, winEvent, true);
        } //END GAME
        else {
            if (game.getAddons().contains(GameAddon.PRIVATE_GAME) && game.playersCount() >= 2 && timer >= 60) {
                DatabaseGameBase.addGame(game, winEvent, true);
            } else {
                DatabaseGameBase.addGame(game, winEvent, true);
            }
